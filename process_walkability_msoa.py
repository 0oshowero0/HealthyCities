...
daily_living_score = pd.read_csv('./temp_output/walkability/daily_living_score/daily_living_score_msoa.csv')[['MSOACode','DailyLivingScore']]
road_intersection_density = pd.read_csv('./temp_output/walkability/road_intersection_density/walking_road_density_msoa.csv')[['MSOACode','RoadIntersectionDensity']]
pop_density = pd.read_csv('./output/environmental_determinants/basic_statistics/population/population_msoa.csv')[['MSOACode','PopulationDensity']]
...
walkability = daily_living_score.merge(road_intersection_density, left_on='MSOACode', right_on='MSOACode')
walkability = walkability.merge(pop_density, left_on='MSOACode', right_on='MSOACode')
...
walkability = msoa_lut.merge(walkability, left_on='MSOACode', right_on='MSOACode')

walkability = walkability.sort_values(['CityCode','MSOACode'], ascending=True)
walkability.to_csv(OUTPUT_PATH.joinpath('walkability_msoa.csv'),index=False)
